...
ACCOUNT = {
    "as_of": "2026-06-02",
    "nav": 7526.00,          # ⚠️ 估算(待 Robinhood 账户页确认):cash 1191.65 + 持仓 + 期权
    "cash": 1191.65,         # 由完整台账轧差估算(6/2 大量买入后);待账户页核对
    "net_deposits": 6675.08, # 历史净入金(含 bonus)
}

# ── 2. 当前持仓最新价(用于未实现盈亏)──────────────────────────────
PRICES = {     # 2026-06-02 收盘附近(WebSearch);CIEN/QCOM 已清仓不列
    "MU": 1059.44, "SPY": 760.00, "DTCR": 30.47, "ASTS": 104.90,
    "BB": 8.78, "ADBE": 274.03, "MRVL": 282.92,
}
...
LEDGER = [
 ...
 # ---- 6/1–6/2 新增 ----
 ("2026-06-01","ADBE","B",3,267.00),
 ("2026-06-02","CIEN","S",1,586.31),   # CIEN 清仓(财报前全出)
 ("2026-06-02","QCOM","S",3,228.45),   # QCOM 清仓
 ("2026-06-02","MRVL","B",3,262.97),   # 新建(AI 半导体,当天 +29%)
 ("2026-06-02","MU","B",1,1022.00),    # MU 加仓 → 2 股(均价 $965.89)
]
# 数据缺口手工修正:LITE2X(首笔买入在 PDF 覆盖范围外,净额 -5.83);TSLA 赠股 +5.65
MANUAL_REALIZED = {"LITE2X": -5.83, "TSLA_bonus": 5.65}
OPTIONS_REALIZED = {"NVDA calls (5/15)": 157.0, "BB $9C 6/12": 52.0}  # 已平仓期权(BB:买$62卖$114)
OPTIONS_OPEN = {"DJT $15C 6/18": 17.0, "NOK $17.5C 6/18": 97.0, "BB $11C 6/26": 88.0}  # 仍持有(按成本)

# 卫星桶主题归类(用于分散度展示)
THEME = {"MU":"AI硬件/存储","ASTS":"太空","SPY":"核心","DTCR":"AI数据中心",
         "BB":"投机/软件","ADBE":"软件(设计)","MRVL":"AI半导体"}
